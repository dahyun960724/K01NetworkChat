...
public class MultiServer {
	...
	public static void main(String[] args) {
		...
		try {
			...
			//서버->클라이언트 측으로 메세지를 전송하기 위한 출력스트림(out)
			out = new PrintWriter(socket.getOutputStream(), true);
			//클라이언트로부터 메세지를 받기위한 입력스트림(in)
			in = new BufferedReader(new 
			InputStreamReader(socket.getInputStream()));
			
			//클라이언트가 보낸 메세지를 라인단위로 읽어온다.
			s = in.readLine();//클라이언트의 번호1을 여기서 읽음
			//읽어온 내용을 콘솔에 출력한다.
			System.out.println("Client에서 읽어옴:"+ s);
			//클라이언트로 응답메세지(Echo)를 보내준다.
			out.println("Server에서 응답:"+ s);//out은 PrintWriter의 객체//번호2
			//콘솔에 종료메세지를 출력한다.
