...
public class MultiServer {
	...
	public static void main(String[] args) {
		...
		String name = "";//클라이언트의 이름을 저장 (챗1에 없는데 챗2에서 추가됨)
		...
		try {
			 ...
			 */
			if(in != null) {
				name = in.readLine(); //클라이언트의 이름을 읽어서 name에 저장한다.
				System.out.println(name +" 접속"); //서버의 콘솔에 출력 후
				out.println("> "+ name +"님이 접속했습니다."); //클라이언트 측으로 Echo해준다.(쏴준다.)
			}
			...
			while(in != null) {
				...
				out.println(">  "+ name +" ==> "+ s); //클라이언트 측으로 Echo(보냄의 뜻)
			}
